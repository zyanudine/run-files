...
------
cdl
dtype=tso500_fastq  # for alignment to host etc.
project=240821_NDX551323_RUO_0008_AHLGC5BGXW_1    # change project name
alg="_TSO500_bwamem_host"
nextseqdx=nextseq33
samples=all
...
nohup bash ./codes/Nextseq-general-XenofilteR-tso500.sh $dtype $project $alg $nextseqdx $samples $run $ext $cptype $cpdir $roi $reffai $control_run $control_name 2>&1 | nohup tee -a tmp/$project.nohup
