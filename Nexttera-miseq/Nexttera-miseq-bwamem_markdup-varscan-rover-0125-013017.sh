# 170127_M00662_0125_000000000-G154Y
...
# mkdir -p seqs/170127_M00662_0125_000000000-G154Y/fq
# cp -pr /mnt/win-hp/base_space/NVS_AML_NexteraXT_Run-4-34925894/*/*fastq.gz seqs/170127_M00662_0125_000000000-G154Y/fq/
# gunzip seqs/170127_M00662_0125_000000000-G154Y/fq/*
...
# 11/14/16 cp from Novartis_Myloid-ampliseq-miseq-bwamem-varscanFLT3-rover-0100-103116.sh, for Nextera markdup-bwamem-varscan/rover, not cutadapt
# 12/30/16 change roi file from run 115
# 1/30/17b change roi file again
# 4/21/17 change bed from NVS_NexteraXT_ROI_JAN2017.bed to NVS_NexteraXT_ROI_0DEC2016.bed, to recompute cov metrics
...
cpdir=Miseq_panel_nexttera
project=170127_M00662_0125_000000000-G154Y    # change project name
#roi=TruSight_MMD_calculated_ROI.merge.bed
roi=NVS_NexteraXT_ROI_0DEC2016.bed
roi_mergesort=NVS_NexteraXT_ROI_0DEC2016.bed
reffai=miseq-hg19-genome.fa
hotspot=CHP2.20131001.hotspots.bed   # does not matter since pairallelecount=n
pairallelecount=n
gn2NM=Nextera_miseq_refRNA_accession_NM_number.xls

nohup bash ./codes/Miseq-general-ROVER-nexttera.sh $dtype $project $alg $samples $run $ext $cptype $cpdir $roi $roi_mergesort $reffai $hotspot $pairallelecount $gn2NM 2>&1 | nohup tee -a tmp/$project.nohup
